...
export const AssetModal: React.FC<AssetModalProps> = ({ asset, onSave, onClose }) => {
  ...
  const [valueStr, setValueStr] = useState(asset ? String(asset.value).replace('.', ',') : '');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!trimmed) { setError('Bitte eine Bezeichnung eingeben.'); return; }
    const value = parseFloat(valueStr.replace(',', '.'));
    if (isNaN(value) || value < 0) { setError('Bitte einen gültigen Betrag eingeben.'); return; }
    const rateRaw = rateStr.trim();
    let interestRate: number | undefined;
    if (rateRaw !== '') {
      const parsed = parseFloat(rateRaw.replace(',', '.'));
      if (isNaN(parsed) || parsed < 0) { setError('Bitte einen gültigen Zinssatz eingeben.'); return; }
      interestRate = parsed;
    }
    onSave({ id: asset?.id ?? generateId(), name: trimmed, category, value, interestRate, date: todayIso(), notes: notes.trim() || undefined });
  };

  return (
    <ModalShell
      title={asset ? 'Anlage bearbeiten' : 'Neue Anlage'}
      subtitle={asset ? 'Anlage aktualisieren' : 'Anlage erfassen'}
      onClose={onClose}
    >
      <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
        ...
        <div>
          <label className="block text-gray-700 dark:text-slate-300 text-sm font-medium mb-1">Bezeichnung</label>
          <input
            type="text"
            value={name}
        ...
        <div>
          <label className="block text-gray-700 dark:text-slate-300 text-sm font-medium mb-1">Kategorie</label>
          <select
            value={category}
        ...
        <div className="flex flex-row gap-3">
          <div className="flex-1">
            <label className="block text-gray-700 dark:text-slate-300 text-sm font-medium mb-1">Aktueller Wert</label>
          ...
          <div className="flex-1">
            <label className="block text-gray-700 dark:text-slate-300 text-sm font-medium mb-1">
              Zinssatz <span className="text-gray-400 dark:text-slate-500 font-normal">(opt.)</span>
            </label>
            <div className="relative">
              <input
                type="text"
        ...
        <div className="flex gap-3 pt-1">
          ...
          <button type="submit" className="flex-1 py-2 rounded-lg text-sm font-medium bg-emerald-600 hover:bg-emerald-700 text-white transition-colors">
