...
interface ManualSellModalProps {
  knownEtfs: DemoEtfDef[];
  onConfirm: (isin: string, lot: SaleLot) => void;
  onClose: () => void;
}

export const ManualSellModal: React.FC<ManualSellModalProps> = ({ knownEtfs, onConfirm, onClose }) => {
    ...
    >
      ...
      >
        ...
        <div className="flex items-start justify-between p-5 border-b border-gray-200 dark:border-slate-700">
          <div>
            <h2 className="text-gray-900 dark:text-white font-semibold text-lg">Verkauf manuell eintragen</h2>
            <p className="text-gray-500 dark:text-slate-400 text-xs mt-0.5">
              Transaktion wird in die FIFO-Historie einsortiert
            </p>
          </div>
          <button
            onClick={onClose}
